...
## 核心原则：墙体必须形成闭合空间
- 面与面之间的墙体端点必须精确对齐（共享同一坐标），不能有缝隙
- 相邻墙体应首尾相连形成连续围合，不能各自孤立
- 每个房间应由闭合的墙体环围成
- 相交墙体的端点坐标必须一致（如 wall_01 的 end 与 wall_02 的 start 相同）
...
2. 墙体端点对齐检查：相交墙体必须使用相同的坐标值（如 wall_A 的 end=[3.0,0.0]，wall_B 的 start=[3.0,0.0]）
3. 先梳理墙体拓扑关系（哪些墙首尾相连），再生成坐标
